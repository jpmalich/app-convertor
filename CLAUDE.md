...
This app also lives on the Emergent platform (https://app.emergent.sh), where its creator maintains a parallel copy. **After completing any feature or change to the application, append a new entry to `PromptsForEmergent.md`** (newest at the bottom): date, one-line summary, and a self-contained copy-pasteable prompt that lets the Emergent AI agent replicate the change without seeing this repo's diffs — describe the change by rule and intent, name the files/components involved, and end with a verification step. New entries start at `Revision: r1` with an `initial` change-log line, and get a row in the sync status table at the top of the file.

**Revisions (required):** before writing a new entry, check whether the completed work changes behavior an EXISTING entry's prompt describes. If so, update that entry's prompt in place AND: bump its `Revision:` line (r1→r2→…), append a change-log line naming what caused the update, update its row in the sync status table, and add/refresh an "If you applied rN…" re-run note. Never silently edit a prompt — the user may have already applied an earlier revision in Emergent and relies on the revision bump to know a re-run is needed. Prompts must stay written so that re-applying the full prompt over an earlier revision is safe.

**Exclusion:** do NOT log anything related to decoupling this repo from Emergent (direct-Anthropic LLM client, Docker self-hosting, Emergent branding/telemetry removal, dependency swaps) — those must never be replicated into Emergent. Repo-only changes (docs, CLAUDE.md, git housekeeping) don't need entries either; only changes that affect the running application do. The "Applied in Emergent?" column in the status table is the user's — never fill it in.
